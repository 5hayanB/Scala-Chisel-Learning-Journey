...
// HARDWIRED TEST:-
// 
// class TestALUTester extends FreeSpec with ChiselScalatestTester{
//     "ALU Tester" in{
//         test(new ALUTester()){c =>
//         c.io.in_A.poke(8.U)
//         c.io.in_B.poke(1.U)
//         c.io.alu_Op.poke(15.U)
//         c.io.out.expect(0.U)
//         }
//     }
// }
    ...
    "ALU Tester" in{
        ...
        for(i <- 0 until 100){
            ...
            val result = aluop match{
                ...
                case ALU_SLT => if(src_a.toInt < src_b.toInt){1}
                                else{0}
                case ALU_SLL => src_a << (src_b & 0x1F)
                case ALU_SLTU => if(src_a < src_b){1}
                                    else{0}
